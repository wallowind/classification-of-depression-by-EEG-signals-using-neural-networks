...
class RNN(CNN_Second):
    def __init__(self, features=(32, 64), kernel_sizes=[(1, 64), (19, 16)],
                 strides=(8, 2), hidden_size=128, **kwargs):
...
class RNN_RIM(CNN_Second):
    def __init__(self, features=(32, 64), kernel_sizes=[(1, 64), (19, 16)],
                 strides=(8, 2), rim_num=16, rim_hid=16, top=4, **kwargs):
        super(RNN_RIM, self).__init__(features, kernel_sizes=kernel_sizes, strides=strides)
        self.rim_layer = RIM(hid=rim_hid, rims=rim_num, sql=21, chl=features[1], top=top)
        self.prediction_layer = torch.nn.Linear(
