...
class Bendr(torch.nn.Module):
    ...
    def forward(self, x: torch.Tensor):
        ...
        for i, m in enumerate(self.ensemble):
            y[i] += m(x[..., i])
        return y.mean(dim=0)
        # return self.predictor(x.sum(dim=2))
    ...
    def load(self, path, strict=True, finetuning=True):
        ...
        # Encoder should be retrained to get grasp over new dataset statistics
        # self.encoder.freeze_features()
        # self.contextualizer.freeze_features()   # Contextualizer is used as is
