...
class CNN_Zero(torch.nn.Module):
    def __init__(self):
        ...
        self.only_layer = torch.nn.Conv2d(
            in_channels=1, out_channels=2,  # out_channels represents two target classes
            kernel_size=(19, 13),   # 19 - number of EEG channels, 13 - length in time
            stride=(1, 6))  # mainly to improve computational efficiency
        # Statistics estimation does not work for unknown reason.
...
class CNN_Second(torch.nn.Module):
    def __init__(self, features=(32, 64), kernel_sizes=[(1, 13), (19, 6)], strides=(6, 2)):
